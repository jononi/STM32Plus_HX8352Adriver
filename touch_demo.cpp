 ...
 * application using an HX8352A driven LCD.
 *
 * Compatible MCU:
 *   STM32F1 HD
 *   STM32F4
...
class Touch_demo : public ThreePointTouchScreenCalibrator::GuiCallback {
  ...
  protected:
    ...
    typedef ITDB02_Landscape_64K<LcdAccessMode> LcdPanel;          // the panel
    //typedef ITDB02_Terminal_Portrait<LcdAccessMode> TerminalAccess;
  ...
  public:
    ...
    void initTouchScreen() {
      ...
      // we've got the PENIRQ attached to GPIOC, port 5. Attach an EXTI line to it and since
    ...
    void calibrate(OutputStream &cr) {
      ...
      _calibrationResults->serialise(cr);
    ...
    void doDemo() {
      ...
      // this output stream is used temporarily to store calibration results
      ByteArrayOutputStream *os = new ByteArrayOutputStream(32,4);

      // calibrate touch screen and save results in this ostream
      calibrate(*os);

      // then get the content & size of the stream in this buffer
      uint8_t* cr = os->getBuffer();
      uint32_t crlen = os->getSize();

      // in real application this buffer is to be feed to this istream
      ByteArrayInputStream *is = new ByteArrayInputStream(cr,crlen);

      // create a new empty three points calibration result
      TouchScreenCalibration* _lcr = new ThreePointTouchScreenCalibration();

      // load the calibration results
      if(_lcr->deserialise(*is) )
    		  _touchScreen->setCalibration(*_lcr);
      else
    	  _touchScreen->setCalibration(*_calibrationResults);
      ...
      _touchScreen->TouchScreenReadyEventSender.insertSubscriber(
          TouchScreenReadyEventSourceSlot::bind(this,&Touch_demo::onTouchScreenReady)
        );
